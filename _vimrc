...
""""自定义配置 {
	let g:config_folder = $VIM .'\MyConfig'
	if g:os == 'windows'
		if	empty(glob(g:config_folder))
			call mkdir(g:config_folder)
		endif
	endif
""""} 自定义配置
...
""""========== VIM Plugins Block start ========== {
	...
	""===== Vundle Setting ===== {
		...
		if	g:os == "windows"
			let g:config_bundle_folder = g:config_folder .'\bundle'
			if	empty(glob(g:config_bundle_folder))
				call mkdir(g:config_bundle_folder)
			endif
			let g:vundle_git = g:config_bundle_folder .'\Vundle.vim'
			"todo:试试用变量替换路径
			"set rtp+=g:vundle_git 	 "Vundle的路径
			set rtp+=$VIM\MyConfig\bundle\Vundle.vim 	 "Vundle的路径
			call vundle#rc(g:config_bundle_folder) "插件的安装路径
			"call vundle#begin() "todo mac试试此命令
		elseif
...
"记录历史文件的位置
let MRU_File=g:config_folder .'/mru_files.txt'
"记录的条数
